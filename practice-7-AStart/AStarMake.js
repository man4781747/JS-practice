function AStarMake(YLen, XLen){
  ...
  this.AddClose = function (y ,x) {
    ...
    if ( y-1>=0 &&
      ...
      this.Ay_Maze[y-1][x].g = 10 + this.Ay_Maze[y][x].g;
      this.Ay_Maze[y-1][x].f = this.Ay_Maze[y-1][x].h+this.Ay_Maze[y-1][x].g;
      this.Ay_Maze[y-1][x].parent = this.Ay_Maze[y][x];
    } else if (this.Ay_Open.indexOf((y-1)*this.XLen+x) != -1) {
      if (this.Ay_Maze[y-1][x].f>(this.Ay_Maze[y][x].f+10)) {
        this.Ay_Maze[y-1][x].parent = this.Ay_Maze[y][x];
      }
    }
    if ( x-1>=0 &&
      ...
      this.Ay_Maze[y][x-1].g = 10 + this.Ay_Maze[y][x].g;
      this.Ay_Maze[y][x-1].f = this.Ay_Maze[y][x-1].h+this.Ay_Maze[y][x-1].g;
      this.Ay_Maze[y][x-1].parent = this.Ay_Maze[y][x];
    } else if (this.Ay_Open.indexOf((y)*this.XLen+x-1) != -1) {
      if (this.Ay_Maze[y][x-1].f>this.Ay_Maze[y][x].f+10) {
        this.Ay_Maze[y][x-1].parent = this.Ay_Maze[y][x];
      }
    }
    if ( y+1<this.YLen &&
  ...
  this.AStarShow = function() {
    ...
    for (let i=0;i<this.YLen;i++){
      for (let j=0;j<this.XLen;j++){
        ...
        fill(0);
        text(this.Ay_Maze[i][j].h, 5+j*100, 15+i*100);
        text(this.Ay_Maze[i][j].g, 85+j*100, 15+i*100);
        text(this.Ay_Maze[i][j].f, 85+j*100, 75+i*100);
        text(i*this.XLen+j, 5+j*100, 75+i*100);
        pop();

        push();
        fill(255,255,0);
        for (let i=0;i<this.Ay_Close.length;i++){
          ellipse(50+(this.Ay_Close[i]%this.XLen)*100,
                  50+floor(this.Ay_Close[i]/this.XLen)*100, 10 ,10);
        }
        pop();
        ...
        fill(0,255,255);
        for (let i=0;i<this.Ay_Open.length;i++){
          ellipse(50+(this.Ay_Open[i]%this.XLen)*100,
                  50+floor(this.Ay_Open[i]/this.XLen)*100, 10 ,10);
        }
        pop();
      }
    }
