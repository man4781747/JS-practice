function Collision_Detection(Player, BulletList){
  for (let i=0;i<BulletList.length;i++) {
    if (Player.PlayerPosition.copy().add(
                       BulletList[i].BulletPosition.copy().rotate(PI)
                     ).mag()<=(BulletList[i].BulletSize+Player.PlayerSize)/2) {
                       BulletList[i].BulletSurvive =false;
                       return true;
                     }
  }
  return false;
